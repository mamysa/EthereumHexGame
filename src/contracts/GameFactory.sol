...
contract HexGameInstance {
	...
	event onGameStarted(address gameInstance, address p1, PieceColor p1color, address p2, PieceColor p2color, uint movenum);
	...
	function placePiece(uint x, uint y, PieceColor color) checkValidPiecePlacement(x, y, color) public {
		gameField[y][x].piece = color;
		onPiecePlaced(x, y, color, msg.sender, moveNumber);
		nextMove();
	}

	function placePieceAndCheckWinningCondition(uint x, uint y, PieceColor color, uint[] positions) public {
	...
	modifier checkValidPiecePlacement(uint x, uint y, PieceColor color) {
